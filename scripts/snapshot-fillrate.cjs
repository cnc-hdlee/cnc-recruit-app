...
// 열: K=날짜  경영기획(L~O) 영업(P~S) 생산(T~W) 연구소(X~AA) 크솔본부(AB~AE) 전체(AF~AI)  각 그룹 = Op,Hi,Re,FR
...
(async()=>{
 ...
 const rowVals=[iso,...blk(G('경영기획본부')),...blk(G('영업본부')),...blk(G('생산본부')),...blk(G('연구소')),...blk(G('크솔본부')),...blk(total)];
 // 3-0) self-heal: 과거/중복 날짜 행 제거(첫 등장만 유지) → "하루 1줄" 영구 보장. K~AE만 손댐(왼쪽표·17행 김범준표 무관)
 {
   const kae=(await s.spreadsheets.values.get({spreadsheetId:F,range:`'${SUM}'!K${DATA_START}:AI400`,valueRenderOption:'UNFORMATTED_VALUE'})).data.values||[];
   ...
   if(keep.length<nonEmpty){
     const rows=keep.map(r=>{const a=r.slice(0,25); while(a.length<25)a.push(''); return a;});
     await s.spreadsheets.values.update({spreadsheetId:F,range:`'${SUM}'!K${DATA_START}:AI${DATA_START+rows.length-1}`,valueInputOption:'USER_ENTERED',requestBody:{values:rows}});
     await s.spreadsheets.values.clear({spreadsheetId:F,range:`'${SUM}'!K${DATA_START+rows.length}:AI400`});
